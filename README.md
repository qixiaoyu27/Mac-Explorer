...
![Mac Explorer 浅色界面](docs/images/explorer-light-0.1.25.png)
...
```sh
/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/qixiaoyu27/Mac-Explorer/703d16c336dee0cb63d22b702a05da90d5833715/scripts/install.sh)"
```
...
```sh
MAC_EXPLORER_ALLOW_UNNOTARIZED=1 /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/qixiaoyu27/Mac-Explorer/703d16c336dee0cb63d22b702a05da90d5833715/scripts/install.sh)"
```
...
```sh
shasum -a 256 Mac-Explorer-0.1.34-mac-arm64.dmg
```
...
| 从图标识别文件 | PDF 首页、图片及 Office 内容缩略图，右下角显示默认应用图标；Office 预览范围由 macOS Quick Look 决定，无法预览时回退系统文件图标。更改默认应用后刷新目录生效 |
| 不打开文件也能看内容 | 文本和图片预览、详细信息窗格、Space 调用 macOS 快速查看 |
| 从其他应用跳到文件 | 接收系统“打开方式”请求：进入文件夹，或定位并选中文件 |

“使用文本编辑”会在独立的 TextEdit 实例中按纯文本打开文件，HTML/RTF 显示源码；不改变 TextEdit 的默认设置。
...
![Mac Explorer 深色界面](docs/images/explorer-dark-0.1.25.png)
...
文件列表的常用快捷键遵循 macOS 访达习惯；输入框内保留标准文本编辑行为。

| 操作 | 快捷键 |
| --- | --- |
| 重命名 | Return（回车）；再次回车确认，Esc 取消 |
| 打开 / 快速查看 | ⌘O 或 ⌘↓ / Space 或 ⌘Y |
| 复制 / 粘贴 / 撤销 | ⌘C / ⌘V / ⌘Z |
...
| `npm run test:office-thumbnails` | Office 内容预览与损坏文件回退 |
