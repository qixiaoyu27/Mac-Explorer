...
try {
  ...
  const open = async () => { await select(); await page.getByRole('button', { name: '移到废纸篓 (⌘⌫)', exact: true }).click(); };
  ...
  await page.waitForFunction(() => document.querySelector('.status-feedback')?.textContent.includes('已移到废纸篓'));
