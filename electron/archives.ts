...
async function native(args: string[], timeout = 15_000) {
  try {
    ...
    if (e.killed || e.code === 'ERR_CHILD_PROCESS_STDIO_MAXBUFFER') throw new Error(tr('压缩包超过读取限制或操作超时。'));
    throw new Error(tr(e.stderr?.trim() || readableError(error)));
  }
}
export async function previewArchive(input: string): Promise<ArchivePreview> {
  const filePath = absolute(input);
  if (!isArchive(filePath) || !(await fs.stat(filePath)).isFile()) throw new Error(tr('请选择压缩文件。'));
  try {
    const output = await native(['list', filePath]);
    return { path: filePath, name: path.basename(filePath), entries: output.split('\n').filter(Boolean).map(line => JSON.parse(line)) };
  } catch (error) { throw new Error(tr('无法预览此压缩包：{0}', readableError(error))); }
}
export async function extractArchive(input: string, destination: string, selected: unknown, namedFolder: boolean): Promise<OperationResult> {
  ...
  if (selected !== null && (!Array.isArray(selected) || !selected.length || selected.length > 10_000 || selected.some(p => typeof p !== 'string' || !p || p.includes('\0') || !archive.entries.some(e => e.path === p || e.path.startsWith(p + '/'))))) throw new Error(tr('无效的压缩包项目选择，请重新打开预览。'));
  const target = await fs.realpath(absolute(destination));
  if (!(await fs.stat(target)).isDirectory()) throw new Error(tr('请选择解压目标文件夹。'));
  ...
  try {
    ...
    for (const name of names) {
      ...
      try {
        ...
        result.errors.push({ path: output, message: exists ? tr('同名项目已存在，已跳过并保留原内容。') : readableError(error) });
      }
    }
    return result;
...
export async function prepareArchiveFile(input: string, member: unknown) {
  const archive = await previewArchive(input);
  if (typeof member !== 'string' || !archive.entries.some(entry => entry.path === member && !entry.directory)) throw new Error(tr('请选择压缩包内的文件。'));
  ...
  try {
    ...
    if (!(await fs.lstat(filePath)).isFile()) throw new Error(tr('无法打开此压缩包项目。'));
...
export async function compressArchive(inputs: string[]): Promise<string> {
  if (!inputs.length || inputs.length > 10_000) throw new Error(tr('请先选择要压缩的文件或文件夹。'));
  ...
  for (const source of sources) {
    if (source === path.parse(source).root) throw new Error(tr('不能压缩整个磁盘。'));
    const stat = await fs.lstat(source);
    if (!stat.isFile() && !stat.isDirectory()) throw new Error(tr('暂不支持压缩链接或特殊文件。'));
  }
  ...
  const stem = sources.length === 1 ? singleDirectory ? path.basename(sources[0]) : path.parse(sources[0]).name : tr('压缩文件');
  validName(stem + '.zip');
  ...
  try {
    ...
    if (stderr.trim()) throw new Error(tr('压缩未完成：{0}', stderr.trim()));
    ...
    throw new Error(tr('同名压缩包过多，请先重命名部分文件。'));
  } finally { await fs.rm(staging, { recursive: true, force: true }); }
}
